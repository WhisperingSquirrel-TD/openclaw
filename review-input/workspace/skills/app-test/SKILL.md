...
3a. **Bounded test-evidence rule (MANDATORY):** run the narrowest relevant test first. Capture command output through `scripts/worker_wrapper.py` when it could be substantial, and return only failing assertions, result counts, elapsed time, and source references—not a raw test log. If diagnosis needs cross-file/log correlation, use a bounded sub-agent with the `scripts/README-context-tools.md` hand-off schema.
