...
6a. **Context-efficient development rule (MANDATORY):** treat repository investigation, generated test output, logs, session history, and sub-agent work as bounded inputs. Before reading/searching, state the exact question and smallest relevant source slice. Use `scripts/README-context-tools.md`: `context_extract.py` for known file slices, `session_query.py` for transcript history, `context_dashboard.py` for session state, and `worker_wrapper.py` for commands. Use an isolated sub-agent for cross-file/noisy investigation only with an explicit source allowlist, exclusions, caps, and compact hand-off schema. Deliberate broad context is allowed when it is genuinely the work object; accidental raw dumps are not.
7. Keep changes understandable and testable.
8. For critical logic and important stateful behavior, default to test-first or test-alongside discipline rather than code-now-tests-later.
9. Prefer explicitness over cleverness.
10. Treat logging/observability as part of the implementation slice itself:

- add route/service/domain logs where they are needed
- make failure paths diagnosable
- prefer structured logs and stable fields over ad hoc strings

11. For every new external/client project, implement both paths consciously:
...
12. For GitHub-backed hosted-preview projects, keep the deployment chain explicit:
...
13. Treat the **preview URL surfacing path** as part of the build, not a later polish item:
...
13. Stop and surface uncertainty instead of silently improvising architecture.
14. Before calling implementation complete, state the git/update path: commit, push, and any deploy/restart requirement.
15. For external/client projects, also state whether the project has been wired into the workspace control plane yet, and if not, what is still missing (registry entry, runnable root, GitHub push route, preview URL discovery, auth/env, GUI action support).
16. **Test/logging-in-slice rule (MANDATORY):** do not treat tests and logging as follow-up polish. For each meaningful build slice, include:

- the tests that prove the slice
- the logging/observability needed to diagnose slice failures
- the evidence that both now exist

17. **Critical-system discipline rule (MANDATORY):** if the system has persistence, workflows/state transitions, gateway/API routes, or operational consequences when wrong, default to:

- repository/unit tests
- integration tests
- explicit failure-path handling
- structured logging for success and failure paths

18. **Chat-usable completion rule (MANDATORY):** if Tom asks to build something so it can be run/used "from chat", do not stop at a backend bridge or control-plane route alone. Completion requires one of:
...
19. **Outcome-over-layer rule:** when Tom's request is phrased as an outcome (e.g. "so you can run it from chat"), treat that outcome as the acceptance criterion. Do not present a lower layer (API, gateway route, config, internal bridge) as the solved thing unless that lower layer actually delivers the requested user-facing capability.
20. **Discriminating-debug rule (MANDATORY):** when an integration still fails after one plausible fix, stop proposing adjacent guesses and isolate the failing layer with the smallest direct test. For a chat/runtime tool path, the required chain is: (a) plugin discovered, (b) tool exposed in live agent tool catalog, (c) tool invocation succeeds, (d) downstream service responds, (e) requested end-to-end action succeeds. After each step, state exactly which layer is now proven and which remains unproven.
21. **No-premature-success rule (MANDATORY):** do not say "usable", "working", "fixed", or equivalent until the exact requested workflow has completed successfully at least once. For this class of task, that means the real requested action must run end-to-end; partial progress (tool visible, API reachable, job submitted, etc.) must be described only as partial progress.
22. **No-gated-shortcut-while-building-ungated-path rule (MANDATORY):** if Tom asks to build a route that avoids a gate/tool/dependency (for example "without exec"), do not use the gated route to complete the user-facing task unless Tom explicitly approves that as a temporary workaround. A gated workaround may be used only for tightly scoped diagnostics to isolate a failing layer, and it must be labelled as temporary containment, not completion.
23. **Gate-open-does-not-change-proof rule (MANDATORY):** if the previously-avoided gate later becomes available during the same build/debug session, do not reinterpret that as permission to prove the system through the gated path. The gate may be used only for permanent rollout actions that make the ungated path live (for example restart/reload/deploy/config uptake) — not for claiming the ungated operator path works. User-facing proof must still come from the ungated route itself after rollout.
24. **Goal-protection rule (MANDATORY):** when debugging a system whose purpose is to remove friction, do not defeat the purpose by bypassing the target path just to get the immediate outcome. Protect the build goal first; if a workaround undermines the architecture Tom asked for, stop and ask before using it.
25. **Source-vs-runtime proof rule (MANDATORY):** when a new tool, route, command, or integration exists in source code, do not treat that as evidence it is live. Before claiming the capability exists or debugging higher layers (session refresh, surface caching, user error), verify all four layers explicitly: (a) source file exists, (b) the current build output contains it, (c) the running runtime/gateway loaded it, and (d) the user-facing surface actually exposes it. For a missing chat tool, the minimum discriminating checks are: source grep, built/dist grep, live runtime/tool-registry/log check, and real tool-surface check. If any layer is unproven, report the lowest failing layer first.
26. **Complex-system documentation sync rule (MANDATORY):** when building a system with multiple layers (for example CRM selection, generation queue, gateway, chat tool, review/send path), do not let the architecture live only in code and chat. Update the canonical design/operating document as the build evolves so it stays truthful about:
...
- Use `reference/APP-ENGINEERING-TDD-LOGGING.md` when deciding the right testing and observability depth.
