...
class BriefingApiClient:
        ...
        try:
            with urllib.request.urlopen(req, timeout=30) as resp:
                ...
                try:
                    ...
                    payload = {"success": False, "error": f"Non-JSON response from website API: {text[:300]}"}
            ...
            return ApiResponse(status_code=-1, payload={"success": False, "error": f"Client exception before/while sending request: {type(e).__name__}: {e}"})
    ...
    def list_briefings(self) -> ApiResponse:
        return self._request("GET", "/api/briefings")
