...
if [ -f "$_SOUL_PLAINTEXT_PATH" ] || [ -L "$_SOUL_PLAINTEXT_PATH" ]; then
    if [ ! -f "$_SOUL_ENC_PATH" ]; then
        fail "[soul] Refusing to remove plaintext SOUL.md because no encrypted vault exists at $_SOUL_ENC_PATH"
    fi
...
# totpWindowMinutes must always be 10 — migrate any other value
if agents.get('totpWindowMinutes') != 10:
    old = agents.get('totpWindowMinutes', 'unset')
    agents['totpWindowMinutes'] = 10
    print(f'totpWindowMinutes: {old} -> 10 (corrected)')
else:
    agents['totpWindowMinutes'] = 10
...
# 200-1000 s, before any output generation.  1800 s (30 min) covers even
# worst-case large-context turns.  We force-assign (not setdefault) so
# re-running install corrects a previously-written low value (e.g. 300).
...
# OLLAMA_MAX_CONTEXT_WINDOW caps the context window written to models.json on every
# gateway start.  llama3.2:3b reports 131072 (needs ~15.9 GiB) which OOMs a Pi 4 8 GB.
# 16384 tokens uses ~2 GiB and fits comfortably alongside the gateway.
printf 'OLLAMA_API_KEY=ollama\nOLLAMA_MAX_CONTEXT_WINDOW=16384\n' \
    > "${HOME}/.config/environment.d/openclaw-ollama.conf"
...
# Install the normal bounded writer used by organisation skills. It is copied
# rather than symlinked because the OpenClaw tool verifies the deployed helper
# is a regular file before invoking it.
SP_WRITER_SRC="$HOME/openclaw/attached_assets/integrations/microsoft/sharepoint_queue_writer.py"
SP_WRITER_DST="$HOME/.openclaw/workspace/scripts/sharepoint-queue-writer.py"
if [ -f "$SP_WRITER_SRC" ]; then
    mkdir -p "$HOME/.openclaw/workspace/scripts"
    install -m 700 "$SP_WRITER_SRC" "$SP_WRITER_DST"
    info "SharePoint bounded writer installed: $SP_WRITER_DST"
else
    warn "SharePoint bounded writer not found at $SP_WRITER_SRC — skill writes will report unavailable"
fi
...
echo ""
echo "  Task-system email dispatch (governed send — no fresh TOTP prompt after owner sign-off):"
echo "    Sender: ~/.openclaw/integrations/microsoft/send.py --task-dispatch-permit <path>"
echo "    Requires the SAME secret in the environment of BOTH this sender AND the"
echo "    separate Workspace Control Panel / task-system service that issues permits:"
echo "      TASK_SYSTEM_EMAIL_DISPATCH_KEY=<32+ random bytes, e.g. \`openssl rand -hex 32\`>"
echo "    This key is never generated or rotated by this installer — it is a shared"
echo "    secret between two independently deployed services. Set it once in"
echo "    ~/.openclaw/.env (for send.py) and in the task-system service's own env,"
echo "    then keep them in sync on rotation. Direct L1 email still uses the normal"
echo "    exec.run TOTP gate and never touches this key. See knowledge/task-system-email-dispatch.md."
echo ""
...
# ---------------------------------------------------------------------------
# Pi reliability hardening (idempotent — safe to re-run on every install)
#   1. Make Workspace Control Panel (WCP) helper scripts executable. They live
#      under ~/.openclaw/workspace/ (runtime data, not deployed from the repo)
#      so they can lose the +x bit — /wcp_up then fails with "WCP helper script
#      is not executable".
#   2. DNS resilience. On this Pi /etc/resolv.conf is Tailscale-managed with
...
#      alternative (add a global resolver in the Tailscale admin console).
# ---------------------------------------------------------------------------

# 1. WCP helper scripts executable
WCP_SCRIPTS_DIR="$HOME/.openclaw/workspace/projects/workspace-control-panel/scripts"
if [ -d "$WCP_SCRIPTS_DIR" ]; then
    if ls "$WCP_SCRIPTS_DIR"/*.sh >/dev/null 2>&1; then
        chmod +x "$WCP_SCRIPTS_DIR"/*.sh 2>/dev/null || true
        info "WCP helper scripts marked executable: $WCP_SCRIPTS_DIR"
    fi
fi

# 2. DNS resilience
if command -v tailscale >/dev/null 2>&1 && [ -f /etc/resolv.conf ]; then
    # Tailscale MagicDNS present AND no public fallback resolver configured?
    if grep -q "generated by tailscale" /etc/resolv.conf 2>/dev/null && \
       ! grep -Eq "nameserver[[:space:]]+(8\.8\.8\.8|8\.8\.4\.4|1\.1\.1\.1|1\.0\.0\.1|9\.9\.9\.9)" /etc/resolv.conf 2>/dev/null; then
        if [ "${OPENCLAW_TAILSCALE_DISABLE_DNS:-0}" = "1" ]; then
            if sudo tailscale set --accept-dns=false 2>/dev/null; then
                info "DNS hardening: Tailscale MagicDNS override disabled (OPENCLAW_TAILSCALE_DISABLE_DNS=1)."
                info "  The Pi now uses your network's DNS — resilient to tailscaled hiccups."
                info "  Note: *.ts.net name resolution on the Pi itself is now disabled."
            else
                warn "DNS hardening requested but 'sudo tailscale set --accept-dns=false' failed."
                warn "  Run it manually once: sudo tailscale set --accept-dns=false"
            fi
        else
            warn "DNS single point of failure: Tailscale MagicDNS (100.100.100.100) is the ONLY resolver."
            warn "  If tailscaled hiccups, ALL DNS fails (Errno -3) and the mgmt-bot loses Telegram."
            warn "  Fix ONE of these (both reboot-safe, applied automatically next time):"
            warn "    A) ZERO TRADEOFF: Tailscale admin console -> DNS -> add a global nameserver"
            warn "       (e.g. 8.8.8.8 or 1.1.1.1). Keeps MagicDNS, just adds a fallback."
            warn "    B) AUTO HERE: add  OPENCLAW_TAILSCALE_DISABLE_DNS=1  to ~/.openclaw/.env"
            warn "       (this install then runs 'tailscale set --accept-dns=false' for you;"
            warn "        the Pi uses network DNS, but *.ts.net names stop resolving on the Pi)."
        fi
    else
        info "DNS check: a public fallback resolver is present or Tailscale isn't managing resolv.conf — OK."
    fi
fi
