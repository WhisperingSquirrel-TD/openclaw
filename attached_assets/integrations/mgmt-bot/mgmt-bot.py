...
def cmd_install(token: str, chat_id: str) -> None:
    ...
    wrapper_path.write_text(
        ...
        "time.sleep(8)  # give systemd a moment to settle after restarts\n"
        "gw = 'openclaw-gateway.service'\n"
        "if not svc_active(gw):\n"
        "    subprocess.run(['systemctl', '--user', 'start', gw], capture_output=True)\n"
        "    time.sleep(5)\n"
        "\n"
        "gw_ok  = svc_active(gw)\n"
        "gw_tag = '✅ Gateway: running' if gw_ok else '❌ Gateway: still inactive — check `journalctl --user -u openclaw-gateway.service -n 30`'\n"
        "\n"
        "if res.returncode == 0:\n"
