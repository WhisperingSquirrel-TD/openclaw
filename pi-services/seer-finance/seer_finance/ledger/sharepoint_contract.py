...
class SharePointDocumentStore:
    ...
    def read_snapshot(self, path: str) -> dict[str, str]:
    ...
    def write(self, *, path: str, content: str, delivery: Mapping[str, Any] | None = None) -> str:
        ...
        if self._is_rebase_required(result):
            self._retire_rebase(operation_id, clean_path, base, desired_hash, result)
            raise SharePointRebaseRequired(
    ...
    def _retire_rebase(
        ...
        desired_hash: str, result: Mapping[str, Any] | None,
    ) -> None:
        with self._lock():
            self._retire_rebase_locked(
                operation_id, path,
