...
4. A guardrail that must apply on _every_ session goes in "Critical rules" below (because this map is always loaded); its detail goes in the topic file it links to.
...
3. **Native Git fetch/pull/merge/push from the workspace CLI is verified and permitted.** Use normal Git sync when needed; GitHub remains authoritative.
4. **Never remove `manage-package-manager-versions=false`** from `.npmrc`. → [architecture.md](./knowledge/architecture.md)
5. **Do not add new cron/background jobs in the 06:xx or 07:xx windows** (CRM 06:00, another job 07:00). Use 08:00+. Pre-existing installer declarations (health 06:55 and Monday AI briefing 06:00) have no approval evidence in this repository; leave them unchanged pending owner/runtime reconciliation and do not treat them as templates for new jobs. → [pi-deployment.md](./knowledge/pi-deployment.md#scheduling-constraint--avoid-06xx-and-07xx)
