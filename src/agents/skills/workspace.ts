 ...
 *
 * Saves ~5â6 tokens per skill path Ã N skills â 400â600 tokens total.
 */
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `â ï¸ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`openclaw skills check\` to audit.`
    : compact
      ? `â ï¸ Skills catalog using compact format (descriptions omitted). Run \`openclaw skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
