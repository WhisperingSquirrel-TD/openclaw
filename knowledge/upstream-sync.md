...
- **`testRegexWithBoundedInput` in `src/security/safe-regex.ts`** — implemented by us (2026-07-12): bounded-input `regex.test` (caps input at 2048 chars, resets `lastIndex`, catches throws). Used by Discord exec-approvals and `src/infra/exec-approval-forwarder.ts`. Upstream still lacks it — preserve on every sync or those modules throw at runtime.
